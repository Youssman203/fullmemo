...
async function connecterEnseignant() {
  try {
    ...
    const response = await axios.post(`${API_BASE}/users/login`, ENSEIGNANT);
    
    if (response.data.success && response.data.token) {
      enseignantToken = response.data.token;
    ...
    console.error('❌ Erreur connexion enseignant:', error.response?.data || error.message);
    throw error;
  }
}
